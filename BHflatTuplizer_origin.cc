...
void BHflatTuplizer_origin(std::string inFilename, std::string outFilename, std::string metListFilename) {
  ...
  for (int iEvent = 0; iEvent < nEvents; ++iEvent) {
    ...
    if (!passed_globalTightHalo2016Filter || !passed_goodVertices || !passed_HBHENoiseFilter || !passed_HBHENoiseIsoFilter || !passed_filterbadPFMuon || !passed_filterbadChCandidate || !passed_EcalDeadCellTriggerPrimitiveFilter) continue;
    //if (!passed_globalSuperTightHalo2016Filter || !passed_goodVertices || !passed_HBHENoiseFilter || !passed_HBHENoiseIsoFilter || !passed_filterbadPFMuon || !passed_filterbadChCandidate || !passed_EcalDeadCellTriggerPrimitiveFilter) continue;
        ...
        if (eventHasElectron) {
          for (int iElectron = 0; iElectron < 25; ++iElectron) {
            ...
            if (EleEt[iElectron]>70.) {
              ...
              for (int iMuon = 0; iMuon < 25; ++iMuon ) {
                if (MuEt[iMuon]>70 && MuPFdBiso[iMuon]<0.15 && dR(EleEta[iElectron],ElePhi[iElectron], MuEta[iMuon], MuPhi[iMuon]) < 0.3) {
                  passIso = false;
                  passIso_tight = false;
                  if (dumpIsoInfo) {
        ...
        if (multiplicity>=3 && ST>7000 && dumpBigEvents) {
          ...
          std::cout << " In run number "<< runno<< " lumi section "<< lumiblock<< " event number "<< evtno<<" :ST is " << ST<< " and multiplicity is " <<multiplicity<<"\n"<<std::endl;
          outTextFile << messageBuffer;
          for (int j=0; j<25; ++j) {
            if(JetEt[j]>0.000) {
              ...
              std::cout << " Jet "<< j<< " has TightJet= "<< TightJets[j]<< " Et= "<< JetEt[j]<<", Px= " << JetPx[j]<< ", Py=" <<JetPy[j]<< ", Eta=" <<JetEta[j]<< ", Phi=" <<JetPhi[j]<<"\n"<<std::endl;
              outTextFile << messageBuffer;
            }
            if (debugFlag) cout  << messageBuffer;
          }
          for (int j=0; j<25; ++j) {
            if(EleEt[j]>0.000) {
              ...
              std::cout << " Ele "<< j<< " has Et= "<< EleEt[j]<<", Px= " << ElePx[j]<< ", Py=" <<ElePy[j]<< ", Eta=" <<EleEta[j]<< ", Phi=" <<ElePhi[j]<<"\n"<<std::endl;
              outTextFile << messageBuffer;
            }
            if (debugFlag) cout  << messageBuffer;
          }
          for (int j=0; j<25; ++j) {
            if(PhEt[j]>0.000) {
              ...
              std::cout << " Ph "<< j<< " has Et= "<< PhEt[j]<<", Px= " << PhPx[j]<< ", Py=" <<PhPy[j]<< ", Eta=" <<PhEta[j]<< ", Phi=" <<PhPhi[j]<<"\n"<<std::endl;
              outTextFile << messageBuffer;
            }
            if (debugFlag) cout  << messageBuffer;
          }
          for (int j=0; j<25; ++j) {
            if(MuEt[j]>0.000) {
              ...
              std::cout << " Mu "<< j<< " has Et= "<< MuEt[j]<<", Px= " << MuPx[j]<< ", Py=" <<MuPy[j]<< ", Eta=" <<MuEta[j]<< ", Phi=" <<MuPhi[j]<<"\n"<<std::endl;
              outTextFile << messageBuffer;
            }
            if (debugFlag) cout  << messageBuffer;
          }
